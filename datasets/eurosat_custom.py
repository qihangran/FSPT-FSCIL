...
CLASSES = ['Annual_Crop_Land', 'Forest', 'Herbaceous_Vegetation_Land', 'Highway_or_Road', 'Industrial_Buildings', 'Pasture_Land','Permanent_Crop_Land',
                      'Residential_Buildings', 'River', 'Sea_or_Lake']



@DATASET_REGISTRY.register()
class EUROSAT_CUSTOM(DatasetBase):


    dataset_dir = "eurosat_custom"
    ...
    def _read_data_train(self, data_dir, num_labeled, val_percent, session=0):
        ...
        for label, class_name in enumerate(class_names):
            ...
            for i, imname in enumerate(imnames_train):
                ...
                item = Datum(impath=impath, label=label+6, classname=CLASSES[label+6])
