...
def _build_transform_train(cfg, choices, target_size, normalize):
    ...
    if all(conditions):
        ...
        # tfm_train += [Resize(input_size, interpolation=interp_mode)] #224x224

        #print(f"+ resize the smaller edge to {max(input_size)}")
        tfm_train += [Resize(max(input_size), interpolation=interp_mode)]#224，smaller edge 224
...
def _build_transform_test(cfg, choices, target_size, normalize):
    ...
    # #-------------------------------------------
    # print(f"+ rgb")
    # tfm_test += [lambda image: image.convert("RGB")]
